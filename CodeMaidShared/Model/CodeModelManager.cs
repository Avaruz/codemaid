...
namespace ASGV.CodeMaid.Model
{
  ...
  internal sealed class CodeModelManager
  {
    ...
    {
      return _instance ??= new CodeModelManager(package);
    }
    ...
    internal SetCodeItems RetrieveAllCodeItems(Document document, bool loadLazyInitializedValues = false)
    {
      ...
      CodeModel codeModel = _codeModelCache.GetCodeModel(document);
    ...
    private void BuildCodeItems(CodeModel codeModel)
    {
      ...
      try
      {
        ...
        SetCodeItems codeItems = _codeModelBuilder.RetrieveAllCodeItems(codeModel.Document);
    ...
    private void LoadLazyInitializedValues(CodeModel codeModel)
    {
      try
      {
        ...
        foreach (BaseCodeItem codeItem in codeModel.CodeItems)
        {
    ...
    {
      ThreadHelper.ThrowIfNotOnUIThread();
      Action<CodeModel> codeModelBuilt = CodeModelBuilt;
