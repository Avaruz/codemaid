  ...
  internal sealed class FindInSolutionExplorerCommand : BaseCommand
  {
    ...
    protected override void OnExecute()
    {
      ...
      if (document != null)
      {
        ...
        Command command = _commandHelper.FindCommand("{D63DB1F0-404E-4B21-9648-CA8D99245EC3}", 36);
    ...
    {
      ThreadHelper.ThrowIfNotOnUIThread();
      IVsUIHierarchyWindow solutionExplorer = VsShellUtilities.GetUIHierarchyWindow(Package, VSConstants.StandardToolWindows.SolutionExplorer);
      IVsWindowSearch ws = solutionExplorer as IVsWindowSearch;
      ws?.ClearSearch();
    }
    ...
    private void ToggleSolutionFoldersOpenTemporarily(UIHierarchyItem parentItem)
    {
      ...
      ThreadHelper.ThrowIfNotOnUIThread();
      Project project = parentItem.Object as Project;
      bool isCollapsedSolutionFolder = project?.Kind == solutionFolderGuid && !parentItem.UIHierarchyItems.Expanded;
