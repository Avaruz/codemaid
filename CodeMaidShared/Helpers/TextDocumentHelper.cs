  ...
  internal static class TextDocumentHelper
  {
    ...
    internal static IEnumerable<EditPoint> FindMatches(TextDocument textDocument, string patternString)
    {
      ...
      UIThread.Run(() =>
      {
        ...
        if (TryGetTextBufferAt(textDocument.Parent.FullName, out ITextBuffer textBuffer))
        {
          ...
          IEnumerable<Span> findMatches = finder.FindAll();
          foreach (Span match in findMatches)
          {
    ...
    internal static IEnumerable<EditPoint> FindMatches(TextSelection textSelection, string patternString)
    {
      ...
      UIThread.Run(() =>
      {
        ...
        {
          IFinder finder = GetFinder(patternString, textBuffer);
          IEnumerable<Span> findMatches = finder.FindAll(GetSnapshotSpanForTextSelection(textBuffer.CurrentSnapshot, textSelection));
          foreach (Span match in findMatches)
          {
    ...
    private static EditPoint GetEditPointForSnapshotPosition(TextDocument textDocument, ITextSnapshot textSnapshot, int position)
    {
      ...
      ITextSnapshotLine textSnapshotLine = textSnapshot.GetLineFromPosition(position);
    ...
    {
      IFindService findService = CodeMaidPackage.Instance.ComponentModel.GetService<IFindService>();
      IFinderFactory finderFactory = findService.CreateFinderFactory(findWhat, StandardFindOptions);
    ...
    {
      IFindService findService = CodeMaidPackage.Instance.ComponentModel.GetService<IFindService>();
      IFinderFactory finderFactory = findService.CreateFinderFactory(findWhat, replaceWith, StandardFindOptions);
      return finderFactory.Create(textBuffer.CurrentSnapshot);
    }
    ...
    private static Span GetSnapshotSpanForTextSelection(ITextSnapshot textSnapshot, TextSelection selection)
    {
      ...
      int startPosition = GetSnapshotPositionForTextPoint(textSnapshot, selection.AnchorPoint);
      int endPosition = GetSnapshotPositionForTextPoint(textSnapshot, selection.ActivePoint);

      return startPosition <= endPosition
              ? new Span(startPosition, endPosition - startPosition)
              : new Span(endPosition, startPosition - endPosition);
    }
    ...
    private static int GetSnapshotPositionForTextPoint(ITextSnapshot textSnapshot, TextPoint textPoint)
    {
      ...
      ITextSnapshotLine textSnapshotLine = textSnapshot.GetLineFromLineNumber(textPoint.Line - 1);
    ...
    private static Span GetSnapshotSpanForExtent(ITextSnapshot textSnapshot, EditPoint startPoint, EditPoint endPoint)
    {
      ...
      int startPosition = GetSnapshotPositionForTextPoint(textSnapshot, startPoint);
      int endPosition = GetSnapshotPositionForTextPoint(textSnapshot, endPoint);
    ...
    private static void ReplaceAll(ITextBuffer textBuffer, IEnumerable<FinderReplacement> replacements)
    {
      if (replacements.Any())
      {
        ...
        {
          foreach (FinderReplacement match in replacements)
          {
    ...
    private static bool TryGetTextBufferAt(string filePath, out ITextBuffer textBuffer)
    {
        ...
        out windowFrame))
      {
        ...
        {
          IVsTextBuffer buffer = lines as IVsTextBuffer;
          if (buffer != null)
          {
            IVsEditorAdaptersFactoryService editorAdapterFactoryService = CodeMaidPackage.Instance.ComponentModel.GetService<IVsEditorAdaptersFactoryService>();
