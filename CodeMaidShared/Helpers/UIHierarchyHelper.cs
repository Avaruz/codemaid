    ...
    internal static class UIHierarchyHelper
    {
        ...
        internal static bool HasExpandedChildren(UIHierarchyItem parentItem)
        {
            ...
            return parentItem == null
                    ? throw new ArgumentNullException(nameof(parentItem))
                    : parentItem.UIHierarchyItems.Cast<UIHierarchyItem>().Any(
                      childItem =>
                      {
                          ThreadHelper.ThrowIfNotOnUIThread();
        ...
        private static bool ShouldCollapseItem(UIHierarchyItem parentItem)
        {
            ...
            {
                Solution solution = parentItem.DTE.Solution;
                if (solution?.Projects.OfType<Project>().All(x =>
                { ThreadHelper.ThrowIfNotOnUIThread(); return x == parentItem.Object || x.Name == "Miscellaneous Files"; }) == true)
                {
                    return false;
                }
            }
