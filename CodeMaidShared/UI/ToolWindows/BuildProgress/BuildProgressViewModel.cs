...
namespace ASGV.CodeMaid.UI.ToolWindows.BuildProgress
{
  ...
  public class BuildProgressViewModel : Bindable
  {
    ...
    /// </summary>
    public DelegateCommand CancelBuildCommand => _cancelBuildCommand ??= new DelegateCommand(OnCancelBuildCommandExecuted, OnCancelBuildCommandCanExecute);
    ...
    private void UpdateTaskbarStatus()
    {
      ...
      if (IsBuildActive)
      {
        progressState = HasBuildFailed
          ? TaskbarItemProgressState.Error
          : IsProgressIndeterminate ? TaskbarItemProgressState.Indeterminate : TaskbarItemProgressState.Normal;
      }
