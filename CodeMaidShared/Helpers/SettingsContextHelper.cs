...
namespace ASGV.CodeMaid.Helpers
{
  ...
  internal sealed class SettingsContextHelper
  {
    ...
    {
      return _instance ??= new SettingsContextHelper(package);
    }
    ...
    internal static string GetSolutionSettingsPath(SettingsContext context)
    {
      ...
      object solutionPath = context["SolutionPath"];
    ...
    internal bool LoadSolutionSpecificSettings(Settings settings, bool canCreate = false)
    {
      ...
      {
        string solutionPath = Path.GetDirectoryName(_package.IDE.Solution.FullName);
        if (!string.IsNullOrWhiteSpace(solutionPath))
        {
          string solutionConfig = Path.Combine(solutionPath, SettingsFilename);
