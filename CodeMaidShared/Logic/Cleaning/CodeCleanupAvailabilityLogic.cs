...
using ASGV.CodeMaid.UI.Dialogs.Prompts;
...
namespace ASGV.CodeMaid.Logic.Cleaning
{
    ...
    internal sealed class CodeCleanupAvailabilityLogic
    {
        ...
        {
            return _instance ??= new CodeCleanupAvailabilityLogic(package);
        }
        ...
        /// </summary>
        private EditorFactory EditorFactory => _editorFactory ??= new EditorFactory();
        ...
        internal bool CanCleanupProjectItem(ProjectItem projectItem)
        {
            ...
            string projectItemFileName = projectItem.GetFileName();
        ...
        private bool HasAutoGeneratedHeader(Document document)
        {
            ...
            if (textDocument != null)
            {
                ...
                EditPoint editPoint = TextDocumentHelper.FirstOrDefaultMatch(textDocument, pattern);
                if (editPoint != null)
                {
                    return true;
                }
        ...
        private bool IsDocumentLanguageIncludedByOptions(Document document)
        {
            ...
            string extension = GetProjectItemExtension(projectItem);
        ...
        private bool IsFileNameExcludedByOptions(string filename)
        {
            ...
            IEnumerable<string> cleanupExclusions = CleanupExclusions;
            return cleanupExclusions != null && cleanupExclusions.Any(cleanupExclusion => Regex.IsMatch(filename, cleanupExclusion, RegexOptions.IgnoreCase));
        }
        ...
        private bool IsFileNameIncludedByOptions(string filename)
        {
            ...
            IEnumerable<string> cleanupInclusions = CleanupInclusions;
            return cleanupInclusions?.Any() != true || cleanupInclusions.Any(cleanupInclusion => Regex.IsMatch(filename, cleanupInclusion, RegexOptions.IgnoreCase));
        }
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            return document != null && IsParentCodeGeneratorExcludedByOptions(document.ProjectItem);
        }
        ...
        {
            ProjectItem parentProjectItem = projectItem?.GetParentProjectItem();
            if (parentProjectItem == null)
            {
                return false;
            }

            string extension = GetProjectItemExtension(parentProjectItem);
            return extension.Equals(".tt", StringComparison.CurrentCultureIgnoreCase) && Settings.Default.Cleaning_ExcludeT4GeneratedCode;
        }
        ...
        private bool IsProjectItemLanguageIncludedByOptions(ProjectItem projectItem)
        {
            ...
            string languageService = EditorFactory.GetLanguageService(extension);
            string languageServiceGuid = languageService?.ToLowerInvariant();
        ...
        private static bool PromptUserAboutCleaningExternalFiles(Document document)
        {
            ...
            {
                ThreadHelper.ThrowIfNotOnUIThread();
                YesNoPromptViewModel viewModel = new()
                {
                ...
                bool? response = UIThread.Run(() =>
                {
                    YesNoPromptWindow window = new() { DataContext = viewModel };
                ...
                {
                    int preference = (int)(response.Value ? AskYesNo.Yes : AskYesNo.No);
