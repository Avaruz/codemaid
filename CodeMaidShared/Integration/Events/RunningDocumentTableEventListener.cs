...
namespace ASGV.CodeMaid.Integration.Events
{
    ...
    internal sealed class RunningDocumentTableEventListener : BaseEventListener, IVsRunningDocTableEvents3
    {
        ...
        public static async Task InitializeAsync(CodeMaidPackage package)
        {
            ...
            await package.SettingsMonitor.WatchAsync<bool>([
        ...
        {
            OnDocumentSaveEventHandler afterSave = AfterSave;
        ...
        {
            OnDocumentSaveEventHandler beforeSave = BeforeSave;
            ThreadHelper.ThrowIfNotOnUIThread();
            if (beforeSave != null)
            {
                Document document = GetDocumentFromCookie(docCookie);
        ...
        private Document GetDocumentFromCookie(uint docCookie)
        {
            ...
            string documentName = RunningDocumentTable.GetDocumentInfo(docCookie).Moniker;
            ThreadHelper.ThrowIfNotOnUIThread();
            // Search against the IDE documents to find the object that matches the full document name.
            return Package.IDE.Documents.OfType<Document>().FirstOrDefault(x =>
            {
                ThreadHelper.ThrowIfNotOnUIThread();
                return x.FullName == documentName;
            });
        }
    }
}