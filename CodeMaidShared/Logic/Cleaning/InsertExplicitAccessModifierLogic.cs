...
namespace ASGV.CodeMaid.Logic.Cleaning
{
  ...
  internal sealed class InsertExplicitAccessModifierLogic
  {
    ...
    {
      return _instance ??= new InsertExplicitAccessModifierLogic();
    }
    ...
    public void InsertExplicitAccessModifiersOnClasses(IEnumerable<CodeItemClass> classes)
    {
      ...
      foreach (EnvDTE80.CodeClass2 codeClass in classes.Select(x => x.CodeClass).Where(y => y != null))
      {
        string classDeclaration = CodeElementHelper.GetClassDeclaration(codeClass);
    ...
    public void InsertExplicitAccessModifiersOnDelegates(IEnumerable<CodeItemDelegate> delegates)
    {
      ...
      foreach (EnvDTE80.CodeDelegate2 codeDelegate in delegates.Select(x => x.CodeDelegate).Where(y => y != null))
      {
        string delegateDeclaration = CodeElementHelper.GetDelegateDeclaration(codeDelegate);
        ThreadHelper.ThrowIfNotOnUIThread();
    ...
    {
      if (!Settings.Default.Cleaning_InsertExplicitAccessModifiersOnEnumerations)
      {
        return;
      }

      foreach (CodeEnum codeEnum in enumerations.Select(x => x.CodeEnum).Where(y => y != null))
      {
        string enumDeclaration = CodeElementHelper.GetEnumerationDeclaration(codeEnum);
        ThreadHelper.ThrowIfNotOnUIThread();
    ...
    {
      if (!Settings.Default.Cleaning_InsertExplicitAccessModifiersOnEvents)
      {
        return;
      }
      ThreadHelper.ThrowIfNotOnUIThread();
      foreach (EnvDTE80.CodeEvent codeEvent in events.Select(x => x.CodeEvent).Where(y => y != null))
      {
        try
        {
        ...
        string eventDeclaration = CodeElementHelper.GetEventDeclaration(codeEvent);
    ...
    public void InsertExplicitAccessModifiersOnFields(IEnumerable<CodeItemField> fields)
    {
      ...
      foreach (EnvDTE80.CodeVariable2 codeField in fields.Select(x => x.CodeVariable).Where(y => y != null))
      {
        try
        {
        ...
        string fieldDeclaration = CodeElementHelper.GetFieldDeclaration(codeField);
    ...
    {
      if (!Settings.Default.Cleaning_InsertExplicitAccessModifiersOnInterfaces)
      {
        return;
      }
      ThreadHelper.ThrowIfNotOnUIThread();
      foreach (EnvDTE80.CodeInterface2 codeInterface in interfaces.Select(x => x.CodeInterface).Where(y => y != null))
      {
        string interfaceDeclaration = CodeElementHelper.GetInterfaceDeclaration(codeInterface);
    ...
    public void InsertExplicitAccessModifiersOnMethods(IEnumerable<CodeItemMethod> methods)
    {
      ...
      foreach (EnvDTE80.CodeFunction2 codeFunction in methods.Select(x => x.CodeFunction).Where(y => y != null))
      {
        ThreadHelper.ThrowIfNotOnUIThread();
        try
        {
        ...
        string methodDeclaration = CodeElementHelper.GetMethodDeclaration(codeFunction);
    ...
    public void InsertExplicitAccessModifiersOnProperties(IEnumerable<CodeItemProperty> properties)
    {
      ...
      foreach (EnvDTE80.CodeProperty2 codeProperty in properties.Select(x => x.CodeProperty).Where(y => y != null))
      {
        try
        {
          // Skip explicit interface implementations.
        ...
        string propertyDeclaration = CodeElementHelper.GetPropertyDeclaration(codeProperty);
    ...
    public void InsertExplicitAccessModifiersOnStructs(IEnumerable<CodeItemStruct> structs)
    {
      ...
      ThreadHelper.ThrowIfNotOnUIThread();
      foreach (EnvDTE80.CodeStruct2 codeStruct in structs.Select(x => x.CodeStruct).Where(y => y != null))
      {
        string structDeclaration = CodeElementHelper.GetStructDeclaration(codeStruct);
