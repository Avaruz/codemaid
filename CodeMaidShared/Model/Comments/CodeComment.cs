...
namespace ASGV.CodeMaid.Model.Comments
{
    ...
    internal class CodeComment
    {
        ...
        public TextPoint Format()
        {
            ...
            ThreadHelper.ThrowIfNotOnUIThread();
            string originalText = _startPoint.GetText(_endPoint);
            Match[] matches = [.. _commentLineRegex.Matches(originalText).OfType<Match>()];
            ...
            string[] lineTexts = [.. matches.Select(m => m.Groups["line"].Value)];
            string commentText = string.Join(Environment.NewLine, lineTexts);
            if (commentText.Contains('<'))
            {
                try
                {
                    XElement xml = XElement.Parse($"<doc>{commentText}</doc>");
        ...
        private EditPoint Expand(TextPoint point, Action<EditPoint> foundAction)
        {
            ...
            do
            {
                ...
                string text = current.GetLine();

                Match match = _commentLineRegex.Match(text);
                if (match.Success)
                {
                    ...
                    string currentPrefix = match.Groups["prefix"].Value.TrimStart();
