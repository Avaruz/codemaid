...
namespace ASGV.CodeMaid.Helpers
{
  ...
  internal static class ProjectItemExtensions
  {
    ...
    internal static ProjectItem GetParentProjectItem(this ProjectItem projectItem)
    {
      ...
      {
        ThreadHelper.ThrowIfNotOnUIThread();
        ProjectItem parentProjectItem = projectItem.Collection?.Parent as ProjectItem;
    ...
    internal static bool IsExternal(this ProjectItem projectItem)
    {
      ...
      {
        ThreadHelper.ThrowIfNotOnUIThread();
        return projectItem.Collection == null || !projectItem.IsPhysicalFile() || projectItem.Collection.OfType<ProjectItem>().All(x =>
        {
          ThreadHelper.ThrowIfNotOnUIThread();
