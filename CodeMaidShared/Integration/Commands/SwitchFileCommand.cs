...
namespace ASGV.CodeMaid.Integration.Commands
{
  ...
  internal sealed class SwitchFileCommand : BaseCommand
  {
    ...
    protected override void OnBeforeQueryStatus()
    {
      ...
      Enabled = canAlterate;
      Text = canAlterate ? Resources.SWitchTo + Path.GetFileName(alternatePath) : Resources.SWitchFile;
    }
    ...
    {
      IEnumerable<string> alternatePaths = GetAlternatePaths(document);
    ...
    private IEnumerable<string> GetAlternatePaths(Document document)
    {
      ...
      if (document != null)
      {
        ...
        {
          foreach (IList<string> rfeGroup in _relatedFileExtensions.Value)
          {
            foreach (string extension in rfeGroup)
            {
              if (path.EndsWith(extension, StringComparison.CurrentCultureIgnoreCase))
              {
                ...
                IEnumerable<string> alternates = from i in rfeGroup
                                 where i != extension
