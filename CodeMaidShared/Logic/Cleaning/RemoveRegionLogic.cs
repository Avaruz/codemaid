...
namespace ASGV.CodeMaid.Logic.Cleaning
{
    ...
    internal sealed class RemoveRegionLogic
    {
        ...
        {
            return _instance ??= new RemoveRegionLogic(package);
        }
        ...
        internal void RemoveRegions(TextDocument textDocument)
        {
            ...
            IOrderedEnumerable<CodeItemRegion> regions = _codeModelHelper.RetrieveCodeRegions(textDocument).OrderByDescending(x => x.StartLine);
        ...
        {
            // Retrieve the regions and put them in reverse order (reduces line number updates during removal).
            IOrderedEnumerable<CodeItemRegion> regions = _codeModelHelper.RetrieveCodeRegions(textSelection).OrderByDescending(x => x.StartLine);
        ...
        internal void RemoveRegions(IEnumerable<CodeItemRegion> regions)
        {
            new UndoTransactionHelper(_package, Resources.CodeMaidRemoveRegions).Run(() =>
            {
                ...
                foreach (CodeItemRegion region in regions.OrderByDescending(x => x.StartLine))
                {
                    RemoveRegion(region);
                }
            });
        ...
        {
            NoneEmptyAll setting = (NoneEmptyAll)Settings.Default.Cleaning_RemoveRegions;
            if (setting == NoneEmptyAll.None)
            {
                return;
            }
        ...
        {
            if (region?.IsInvalidated != false || region.IsPseudoGroup || region.StartLine <= 0 || region.EndLine <= 0)
            {
                return;
            }
            ...
            {
                ThreadHelper.ThrowIfNotOnUIThread();
                EditPoint end = region.EndPoint.CreateEditPoint();
                end.StartOfLine();
                ...
                EditPoint start = region.StartPoint.CreateEditPoint();
                start.StartOfLine();
