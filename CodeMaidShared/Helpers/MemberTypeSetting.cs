...
namespace ASGV.CodeMaid.Helpers
{
  ...
  public class MemberTypeSetting : Bindable
  {
    ...
    public static explicit operator MemberTypeSetting(string serializedString)
    {
      ...
      try
      {
        Match match = Regex.Match(serializedString, pattern);

        string defaultName = match.Groups["defaultName"].Value;
        int order = Convert.ToInt32(match.Groups["order"].Value);
        string effectiveName = match.Groups["effectiveName"].Value;
