...
namespace ASGV.CodeMaid.Helpers
{
  ...
  internal static class SolutionHelper
  {
    ...
    {
      List<ProjectItem> selectedProjectItems = new();
      IEnumerable<UIHierarchyItem> selectedUIHierarchyItems = UIHierarchyHelper.GetSelectedUIHierarchyItems(package);
      ThreadHelper.ThrowIfNotOnUIThread();
      foreach (object item in selectedUIHierarchyItems.Select(uiHierarchyItem =>
      {
        ThreadHelper.ThrowIfNotOnUIThread();
    ...
    {
      ThreadHelper.ThrowIfNotOnUIThread();
      IEnumerable<ProjectItem> allItems = GetAllItemsInSolution<ProjectItem>(package.IDE.Solution);
    ...
    private static IEnumerable<object> GetChildren(object parentItem)
    {
      ...
      Solution solution = parentItem as Solution;
      if (solution?.Projects != null)
      {
        return [.. solution.Projects.Cast<Project>().Cast<object>()];
      }

      // Next check if the item is a project.
      Project project = parentItem as Project;
      if (project?.ProjectItems != null)
      {
        return [.. project.ProjectItems.Cast<ProjectItem>().Cast<object>()];
      }
      ...
      if (parentItem is ProjectItem projectItem)
      {
        ...
        {
          return [.. projectItem.ProjectItems.Cast<ProjectItem>().Cast<object>()];
        }
