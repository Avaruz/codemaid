...
namespace ASGV.CodeMaid.Logic.Cleaning
{
    ...
    internal sealed class UpdateLogic
    {
        ...
        {
            return _instance ??= new UpdateLogic(package);
        }
        ...
        internal void UpdateEventAccessorsToBothBeSingleLineOrMultiLine(IEnumerable<CodeItemEvent> events)
        {
            ...
            foreach (CodeItemEvent item in events)
            {
        ...
        internal void UpdatePropertyAccessorsToBothBeSingleLineOrMultiLine(IEnumerable<CodeItemProperty> properties)
        {
            ...
            ThreadHelper.ThrowIfNotOnUIThread();
            foreach (CodeItemProperty item in properties)
            {
        ...
        internal void UpdateSingleLineMethods(IEnumerable<CodeItemMethod> methods)
        {
            ...
            ThreadHelper.ThrowIfNotOnUIThread();
            foreach (CodeItemMethod singleLineMethod in methods.Where(x =>
            {
                ThreadHelper.ThrowIfNotOnUIThread();
                return x.StartPoint.Line == x.EndPoint.Line && x.OverrideKind != vsCMOverrideKind.vsCMOverrideKindAbstract && x.CodeFunction.Parent is not CodeInterface;
            }))
            {
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            EditPoint start = method.StartPoint.CreateEditPoint();
            EditPoint end = method.EndPoint.CreateEditPoint();
        ...
        private void SpreadSingleLineMethodOntoMultipleLines(CodeFunction method)
        {
            ...
            try
            {
                EditPoint start = method.GetStartPoint(vsCMPart.vsCMPartBody).CreateEditPoint();
                EditPoint end = method.GetEndPoint(vsCMPart.vsCMPartBody).CreateEditPoint();
        ...
        private void UpdateAccessorsToBothBeSingleLineOrMultiLine(CodeFunction first, CodeFunction second)
        {
            ...
            CodeFunction multiLineMethod = isFirstSingleLine ? second : first;
            CodeFunction singleLineMethod = isFirstSingleLine ? first : second;

            try
            {
                EditPoint multiLineBodyStart = multiLineMethod.GetStartPoint(vsCMPart.vsCMPartBody).CreateEditPoint();
                EditPoint multiLineBodyEnd = multiLineMethod.GetEndPoint(vsCMPart.vsCMPartBody).CreateEditPoint();
