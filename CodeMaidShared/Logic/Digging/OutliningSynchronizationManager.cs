...
namespace ASGV.CodeMaid.Logic.Digging
{
    /// <summary>
    ...
    internal class OutliningSynchronizationManager : IDisposable
    {
        ...
        private void OnCodeItemParentIsExpandedChanged(object sender, EventArgs eventArgs)
        {
            ...
            {
                ICollapsible iCollapsible = FindCollapsibleFromCodeItemParent(codeItemParent);
        ...
        private void OnCodeRegionsCollapsed(object sender, RegionsCollapsedEventArgs e)
        {
            ...
            {
                ICodeItemParent codeItemParent = FindCodeItemParentFromCollapsible(collapsedRegion);
        ...
        {
            foreach (ICollapsible expandedRegion in e.ExpandedRegions)
            {
                ICodeItemParent codeItemParent = FindCodeItemParentFromCollapsible(expandedRegion);
                if (codeItemParent != null)
                {
        ...
        {
            int startLine = GetStartLineForCollapsible(collapsible);
        ...
        private ICollapsible FindCollapsibleFromCodeItemParent(ICodeItemParent parent)
        {
            ...
            try
            {
                Microsoft.VisualStudio.Text.ITextSnapshotLine snapshotLine = _wpfTextView.TextBuffer.CurrentSnapshot.GetLineFromLineNumber(parent.StartLine);
                IEnumerable<ICollapsible> collapsibles = _outliningManager.GetAllRegions(snapshotLine.Extent);
        ...
        {
            Microsoft.VisualStudio.Text.SnapshotPoint startPoint = collapsible.Extent.GetStartPoint(collapsible.Extent.TextBuffer.CurrentSnapshot);
            int line = startPoint.Snapshot.GetLineNumberFromPosition(startPoint.Position) + 1;
        ...
        {
            IVsTextView textView = GetTextView(document);
            return textView != null && _editorAdaptersFactoryService != null ? _editorAdaptersFactoryService.GetWpfTextView(textView) : null;
        }
        ...
        private IVsTextView GetTextView(Document document)
        {
            ...
            return document == null
              ? null
              : VsShellUtilities.IsDocumentOpen(_package, document.FullName, Guid.Empty, out IVsUIHierarchy hierarchy, out uint itemID, out IVsWindowFrame windowFrame)
        ...
        private static IEnumerable<ICodeItemParent> RecursivelyGetAllCodeItemParents(SetCodeItems codeItems)
        {
            ...
            List<ICodeItemParent> parents = [.. codeItems.OfType<ICodeItemParent>()];
        ...
        {
            foreach (ICodeItemParent codeItemParent in _codeItemParents ?? [])
            {
                ICollapsible iCollapsible = FindCollapsibleFromCodeItemParent(codeItemParent);
                if (iCollapsible != null)
                {
