...
namespace ASGV.CodeMaid.Model
{
  ...
  internal sealed class CodeModelBuilder
  {
    ...
    {
      return _instance ??= new CodeModelBuilder(package);
    }
    ...
    private FileCodeModel RetrieveFileCodeModel(ProjectItem projectItem)
    {
      ...
      if (containingProject != null && containingProject.Kind?.ToLowerInvariant() == sharedProjectTypeGUID)
      {
        System.Collections.Generic.IEnumerable<ProjectItem> similarProjectItems = SolutionHelper.GetSimilarProjectItems(_package, projectItem);
        FileCodeModel fileCodeModel = similarProjectItems.Select(x =>
        {
          ThreadHelper.ThrowIfNotOnUIThread(); return x.FileCodeModel;
    ...
    {
      ThreadHelper.ThrowIfNotOnUIThread();
      BaseCodeItemElement parentCodeItem = FactoryCodeItems.CreateCodeItemElement(codeElement);
