...
namespace ASGV.CodeMaid.Integration.Commands
{
  ...
  internal sealed class CommentFormatCommand : BaseCommand
  {
    ...
    protected override void OnExecute()
    {
      ...
      if (activeTextDocument?.Selection != null)
      {
        string prefix = CodeCommentHelper.GetCommentPrefix(activeTextDocument);
        if (prefix != null)
        {
          ...
          Package.IDE.StatusBar.Text = foundComments
            ? Resources.CodeMaidFinishedFormattingTheComment
            : string.Format(
                  foundComments
