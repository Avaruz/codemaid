...
namespace ASGV.CodeMaid.Helpers
{
    ...
    public class CodeMaidSettingsProvider : LocalFileSettingsProvider
    {
        ...
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection properties)
        {
            try
            {
                string solutionSettingsPath = SettingsContextHelper.GetSolutionSettingsPath(context);
                string userSettingsPath = SettingsContextHelper.GetUserSettingsPath();
                ...
                SettingElementCollection solutionSettings = ReadSettingsFromFile(solutionSettingsPath, sectionName);

                SettingsPropertyValueCollection propertyValues = MergeSettingsIntoPropertyValues(userSettings, solutionSettings, properties);
        ...
        public override void SetPropertyValues(SettingsContext context, SettingsPropertyValueCollection values)
        {
            try
            {
                string settingsPath = SettingsContextHelper.GetSolutionSettingsPath(context) ?? SettingsContextHelper.GetUserSettingsPath();
                string sectionName = GetSectionName(context);
                ThreadHelper.ThrowIfNotOnUIThread();
        ...
        {
            return context == null ? throw new ArgumentNullException(nameof(context)) : (context["GroupName"]?.ToString());
        }
        ...
        private static Configuration GetConfiguration(string path)
        {
            ...
            ExeConfigurationFileMap fileMap = new() { ExeConfigFilename = path };
        ...
        private static SettingElementCollection ReadSettingsFromFile(string path, string sectionName)
        {
            try
            {
                if (!string.IsNullOrEmpty(path) && !string.IsNullOrEmpty(sectionName))
                {
                    ...
                    SettingElementCollection settings = GetSettingElementCollection(config, sectionName);
        ...
        {
            XmlElement node = new XmlDocument().CreateElement("value");
