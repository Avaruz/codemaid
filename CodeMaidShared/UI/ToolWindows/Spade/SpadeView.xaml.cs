...
namespace ASGV.CodeMaid.UI.ToolWindows.Spade
{
    ...
    public partial class SpadeView
    {
        ...
        /// </summary>
        private CodeReorganizationManager CodeReorganizationManager => _codeReorganizationManager ??= CodeReorganizationManager.GetInstance(ViewModel.Package);

        /// <summary>
        /// Gets the lazy-initialized scroll viewer.
        /// </summary>
        private ScrollViewer ScrollViewer => _scrollViewer ??= this.FindVisualChild<ScrollViewer>();
        ...
        private void OnPreviewMouseWheel(object sender, MouseWheelEventArgs e)
        {
            if (Keyboard.Modifiers == ModifierKeys.Control && e.Delta != 0)
            {
                ...
                double percentage = 1 + (0.05 * steps);
        ...
        private void OnTreeViewItemKeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Source is not TreeViewItem treeViewItem || Keyboard.Modifiers != ModifierKeys.None)
            {
                return;
            }
        ...
        private void OnTreeViewItemHeaderMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            TreeViewItem treeViewItem = FindParentTreeViewItem(e.Source);
        ...
        private void OnTreeViewItemHeaderMouseMove(object sender, MouseEventArgs e)
        {
            ...
            IList<TreeViewItem> selectedTreeViewItems = GetSelectedTreeViewItemsIncluding(_dragCandidate);

            foreach (TreeViewItem selectedTreeViewItem in selectedTreeViewItems)
            {
        ...
        private void OnTreeViewItemHeaderMouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            BaseCodeItem baseCodeItem = treeViewItem.DataContext as BaseCodeItem;
        ...
        private void OnTreeViewItemHeaderDragEvent(object sender, DragEventArgs e)
        {
            ...
            TreeViewItem targetTreeViewItem = FindParentTreeViewItem(sender);

            if (targetTreeViewItem != null &&
                e.Data.GetDataPresent(typeof(IList<BaseCodeItem>)))
            {
                IList<BaseCodeItem> codeItemsToMove = e.Data.GetData(typeof(IList<BaseCodeItem>)) as IList<BaseCodeItem>;

                if (targetTreeViewItem.DataContext is BaseCodeItem baseCodeItem && codeItemsToMove?.Contains(baseCodeItem) == false &&
        ...
        private void OnTreeViewItemHeaderDrop(object sender, DragEventArgs e)
        {
            ...
            TreeViewItem treeViewItem = FindParentTreeViewItem(sender);
            if (treeViewItem == null || ReferenceEquals(e.Source, treeViewItem))
            {
                return;
            }

            if (treeViewItem.DataContext is not BaseCodeItem baseCodeItem)
            {
                return;
            }

            if (e.Data.GetData(typeof(IList<BaseCodeItem>)) is not IList<BaseCodeItem> codeItemsToMove)
            {
                return;
            }
            ...
            {
                case DropPosition.Above:
                    foreach (BaseCodeItem codeItemToMove in codeItemsToMove.OrderBy(x => x.StartLine))
                    {
                ...
                case DropPosition.Below:
                    foreach (BaseCodeItem codeItemToMove in codeItemsToMove.OrderByDescending(x => x.StartLine))
                    {
        ...
        private IList<TreeViewItem> GetSelectedTreeViewItemsIncluding(TreeViewItem treeViewItem)
        {
            ...
            List<TreeViewItem> selectedTreeViewItems = [.. treeView.FindVisualChildren<TreeViewItem>().Where(TreeViewMultipleSelectionBehavior.GetIsItemSelected)];
        ...
        {
            FrameworkElement header = targetElement.Template.FindName("PART_HeaderBorder", targetElement) as FrameworkElement;
            double targetHeight = header?.ActualHeight ?? targetElement.ActualHeight;
            Point dropPoint = e.GetPosition(targetElement);
        ...
        {
            return item is ICodeItemParent itemAsParent && (itemAsParent.Children.Contains(baseItem) ||
        ...
        {
            SpadeViewModel viewModel = ViewModel;
        ...
        private void ShowContextMenu(Point point)
        {
            if (ViewModel?.Package is var package)
            {
                package.JoinableTaskFactory.RunAsync(async () =>
                {
                    ...
                    {
                        CommandID contextMenuCommandID = new(PackageGuids.GuidCodeMaidMenuSet, PackageIds.MenuIDCodeMaidContextSpade);
